<?php namespace Pixney\FiskeTheme\Listener;
...
class ApplySorting
{
    ...
    public function handle(SortNavigation $event)
    {
        ...
        if ($settings = $this->settings->value('pixney.theme.fiske::navigation')) {
            $navigation = array_merge(array_flip(unserialize($settings)), $navigation);
        }

        if ($preferences = $this->preferences->value('pixney.theme.fiske::navigation')) {
